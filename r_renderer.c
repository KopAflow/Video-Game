...
void R_Init(SDL_Window *main_win, game_state_t *game_state)
{
    ...
    // Step 2: Initialize your screen buffer and logic
    R_InitScreen(scrnw, scrnh);

    // Note: We are no longer using SDL_Renderer functions
    // SDL_RenderSetLogicalSize() is part of the 2D renderer API,
    // so it should be removed or replaced with OpenGL viewport logic.

    // You can set viewport manually if needed:
    // glViewport(0, 0, scrnw, scrnh);  <-- only after including <GL/gl.h> and loading OpenGL
}
